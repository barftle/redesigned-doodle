...
collatz:
  ...
  .start:
	...
	JA .if  		; Wenn Parameter 1 größer 1, dann spring zu .if sonst nicht.
	MOV rax,rcx  	; rcx wird nach rax getan, damit k ausgegeben wird.
	RET				; rax (k) wird ausgegeben

  .if:
	MOV rax, rdi 	; Der erste Parameter tut man auf rax.
	SHR rax,1 		; Die Zahl in rax wird durch 2 geteilt.
	JC .else 		; wenn carry Flag kommt (beim teilen von Zahl wird ein Bit übertragen, wegen dem Rest, in das nächsthöchere Bit) springt zur Funktion _else sonst nicht.
	SHR rdi,1 		; rdi wird um zwei geteilt.
	INC rcx 		; rcx = rcx + 1 (rcx entspricht k)
	JMP .start 		; springt zurück zur Funktion .start zurück.
  ...
  .else:
	...
	MUL r11 		; rax = rax * r11
	INC rax 		; rax = rax + 1
	MOV rdi,rax 	; rax wird nach rdi getan damit die Schleife funktioniert.
